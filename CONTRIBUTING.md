...
- **`EnemyHitReaction`** (Node) — gắn cạnh `HealthComponent` của quái: flash trắng khi `damaged`, khi `died` thì đóng băng quái + tắt collision + phát `Events.enemy_died` + mờ dần (chạy anim "dead" nếu có) rồi `queue_free`.
- **`EnemyStats`** (Resource) — thông số AI/di chuyển của 1 loại quái (`.tres` riêng mỗi loại), gán vào `EnemyBase.stats`.

Thêm 1 loại quái mới: tạo scene mới `extends EnemyBase` (hoặc kế thừa `pig.tscn`), 1 `<loại>_stats.tres`, bake SpriteFrames (anim tên `idle`/`run`/`hit`/`attack`/`dead`). Không viết FSM mới — chỉnh `behavior` + `stats`.
