...
**Boss** — `BossBase` (`objects/bosses/boss_base.gd`, `class_name`, `CharacterBody2D`, groups `"enemy"`+`"boss"`): its own FSM (`INTRO`/`THINK`/`ATTACK`/`RECOVER`/`HURT`/`DEAD`), no coroutines — patterns are timer/step driven. Reuses `HealthComponent`/`Hurtbox`/`Hitbox` + `EnemyStats`; forwards HP → `Events.boss_health_changed`, phase → `Events.boss_phase_changed`, death → `Events.boss_defeated(boss_id)`. **King Pig** (`objects/bosses/king_pig/king_pig.tscn`, hp 10) — P4a pattern: `bomb_toss` (spawns an `objects/bomb/bomb.tscn` `Bomb` that arcs to the player and explodes). `BodyHitbox` = always-on contact damage. Arena `levels/boss_forest/boss_forest.tscn` (`boss_arena.gd extends LevelBase`, no goal flag — win on `boss_defeated`); registered as `boss_forest` in `LevelData`, unlocked after level 5. `ui/boss_health_bar/` listens to the `Events` boss signals.
