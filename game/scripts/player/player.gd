...
func _build_sprite_frames() -> void:
	...
	for anim_name in CharacterData.ANIMATION_FILES.keys():
		var file_name: String = CharacterData.ANIMATION_FILES[anim_name]
		var path: String = CharacterData.get_animation_path(character, file_name)
		var tex: Texture2D = load(path)
		var do_loop: bool = anim_name == "idle" or anim_name == "run"
		SpriteUtils.add_animation_from_strip(frames, anim_name, tex, FRAME_SIZE, 10.0, do_loop)
...
func hit(force_reposition: bool = false) -> void:
	...
	if GameManager.has_checkpoint:
		...
		SceneTransition.goto("res://scenes/ui/end_screen.tscn")
