...
var _first_button: TextureButton = null
...
func _populate() -> void:
	for child in characters_box.get_children():
		child.queue_free()
	var current: String = SaveManager.get_setting("character", GameManager.selected_character)
	for character_name: String in CharacterData.NAMES:
		characters_box.add_child(_make_card(character_name, character_name == current))

func _make_card(character_name: String, is_current: bool) -> Control:
	...
	if is_current or _first_button == null:
		_first_button = button

	var label := Label.new()
	label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	label.text = ("★ " if is_current else "") + CharacterData.get_display(character_name)
